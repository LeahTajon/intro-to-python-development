"""
File: w06_Prove_AdventureGame.py
Author: Leah Tajon
...
while level_1.lower() != 'match' and level_1.lower() != 'flashlight':
    print(f"\nSorry, {level_1.upper()} is not on the list. Please try again.\n")
    print("You are walking through a dark forest and find two items: a MATCH and a FLASHLIGHT.\n"
    "Which one do you want to pick up?\n")
    ...
    if level_1.lower() == 'match':
        ...
        level_2 = input("\nRUN, HIDE or FIGHT? >>> ")
        while level_2.lower() != 'run' and level_2.lower() != 'hide' and level_2.lower() != 'fight':
            print(f"\nSorry, {level_2.upper()} is not on the list. Please try again.\n")
            ...
            if level_2.lower() == 'run':
                ...
                level_3 = input("\nYES or NO? >>> ")
                while level_3.lower() != 'yes' and level_3.lower() != 'no':
                    print(f"\nSorry, {level_3.upper()} is not on the list. Please try again.\n")
                ...
                while level_3.lower() != 'red' and level_3.lower() != 'yellow':
                    print(f"\nSorry, {level_3.upper()} is not on the list. Please try again.\n")
                    print("While in hiding, you notice that the tree is not an ordinary tree\n"
                    "for it bears two types of fruits, a red fruit and a yellow fruit.\n"
                    "You like to try them both but which one do you want to eat first?\n"
                    "The RED fruit or the YELLOW fruit?")
        ...
        level_2 = input("\nFOLLOW or LOOK? >>> ")
        while level_2.lower() != 'follow' and level_2.lower() != 'look':
            print(f"\nSorry, {level_2.upper()} is not on the list. Please try again.\n")
            print("You pickup the flashlight and turn in on.\nYou see the pathway lit up in front of you,"
            "\nbut you thought you also heard something off to the side.\nDo you want to FOLLOW the path or LOOK in the trees for"
            "the thing that made the noise?")
            ...
            if level_2.lower() == 'follow':
                ...
                while level_3.lower() != 'left' and level_3.lower() != 'right':
                    print(f"\nSorry, {level_3.upper()} is not on the list. Please try again.\n")
                    print("While walking down the path, you notice a key on the ground.\n"
                    "You pick it up, wipe the dust off and put it in your pocket.\n"
                    "When you reach the end of the path, you notice the two doors in front of you.\n"
                    "The first door is on the RIGHT and the other one is on the LEFT.\n"
                    "Both of them are locked so you get the key in your pocket.\n"
                ...
                while level_3.lower() != 'kill' and level_3.lower() != 'feed':
                    print(f"\nSorry, {level_3.upper()} is not on the list. Please try again.\n")
                    print("You look in the trees and see a wild boar feeding on leaves.\n"
                    "Will you KILL the boar or FEED it with other leaves and grass?\n")
